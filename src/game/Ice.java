...
while (i < platformCount) {
    ...
    private void buildBeachFloor(int cols, int floorTopY) {
        ...
        while (c < cols) {
            ...
            if (c == 0) {
                top.setSprite(Assets.iceFloorTopLeft);
            } else if (c == cols - 1) {
                top.setSprite(Assets.iceFloorTopRight);
            } else {
                top.setSprite(Assets.iceFloorTopMid);
            }
            tiles.add(top);

            Tile bot = new Tile(x, floorTopY + TILE, TILE, TILE, true);
            if (c == 0) {
                bot.setSprite(Assets.iceFloor);
            } else if (c == cols - 1) {
                bot.setSprite(Assets.iceFloor);
            } else {
                bot.setSprite(Assets.iceFloor);
    ...
    private void addNormalPlatformWide(int x, int y, int tilesWide) {
        ...
        while (i < tilesWide) {
            ...
            if (i == 0) {
                t.setSprite(Assets.iceFloorTopLeft);
            } else if (i == tilesWide - 1) {
                t.setSprite(Assets.iceFloorTopRight);
            } else {
                t.setSprite(Assets.iceFloorTopMid);
            }
