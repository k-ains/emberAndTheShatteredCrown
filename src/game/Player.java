...
public class Player extends GameObject {
    ...
    private boolean slidingLeft; // true if slope left, false if slope right
    private float slideVelX;
    private float slideVelY;
    private static final float SLIDE_SPEED_Y = 1.2f;  // vertical sliding speed
    private static final float SLIDE_SPEED_X = 1.0f;  // horizontal sliding speed along slope
    ...
    public void update(SimpleLevel level) {
        if (!dead) {
            ...
            while (tIndex < tileList.size()) {
                ...
                if (t.isSolid() && !(t instanceof SlopeTile)) {
            ...
            if (onSlope) {
                int slideX = 1; // horizontal sliding speed
                int slideY = 1; // vertical sliding speed

                x += slopeLeft ? -slideX : slideX;
                y += slideY;

                velY = slideY;   // small downward speed
                onGround = true; // player is "on the slope" for collisions
                // Do NOT block jump: player can still jump while sliding
            }
